...
public class EventDataObjectDeserializer {
   ...
   * Event.EventData#getObject()}. When the optional is present, the deserialized {@code
   * StripeObject} preserves high data integrity because of correspondence between schema of the API
   * response and the model class (the underlying concrete class for abstract {@code StripeObject})
   * schema. This is when {@link Event#getApiVersion()} matches {@link Stripe#API_VERSION}.
